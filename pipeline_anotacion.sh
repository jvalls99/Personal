...
then
    ...
    do        
        ...
        # Annotate the genome
        prokka --species "Klebsiella pneumoniae"  --outdir $anotaciones_path $newfile_path --cpus 6
    done
fi
...
if [ $plasmidos == "no" ]; then
    panaroo --input "gff_files.txt" -t 6 --clean-mode "sensitive" --aligner mafft -a core --core_threshold 0.95 -o "panaroo_output"
fi

if [ $plasmidos == "yes" ]; then
    panaroo --input "gff_files.txt" -t 6 --clean-mode "sensitive" --aligner mafft -a pan --core_threshold 0.95 -o "panaroo_output"
fi
